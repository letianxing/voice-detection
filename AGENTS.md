...
## 现场阈值标定：整套阈值与当前硬件不匹配（2026-09-16，最新）
用户反馈「判断非常模糊」「第二句不回」「回同一句」「惊跳没了」「两人对话转向它也不回」。
从 hri-memory 的 brain_trace 和 .run/attention-decisions.jsonl 取实测值，结论是同一件事：
**每一句用户发言都被 gate_rejected / attention_not_listening 拒绝了**，
那几次「回复」根本不是在回答用户，是 INVITED 邀请路径触发的空 user_text 回复，
所以内容永远相同（「天行，我在呢，想聊什么呀？」）。三个症状同一根因。
现场实测 vs 系统阈值：
  face_confidence 0.81–0.83  vs 要求 ≥0.85   -> 人从来不算「可见」
  gaze_score      0.42–0.45  vs 要求 ≥0.65   -> 注视路径永不触发
  body_facing     0.49–0.52  vs 要求 ≥0.60
  lip_motion      0/23 帧     -> 跨模态绑定永不成立
  rms_dbfs 最大    -40.5      vs 惊跳 ≥-12    -> 267 帧一次未达
  声纹相似度        0.50–0.59  vs 可靠 ≥0.60   -> finalize_gate 的声纹路径全部失败
已修（有实测依据的）：
- 人脸可见度 0.85 -> 0.80。YuNet 检测器自身就配置在 0.80，下游要求比检测器还严且无独立理由，
  这一个数字同时废掉了「人可见」、唇动有效性（lip_landmarks 同样写死 0.85）、跨模态绑定、起始脉冲。
  两处统一为 0.80 并可用 ATTENTION_FACE_CONFIDENCE / VISION_LIP_FACE_SCORE 覆盖。
- 惊跳绝对门限 -12 dBFS -> -45（按 Sipeed 实测调，用户授权）。真正的判别器是 startle_delta_db=24
  （相对自适应噪声底的跃升），绝对门限只是兜底，但 -12 比判别器还严 25 dB，等于它一个人说了算。
  实测验证：安静 -61 不触发、正常说话峰值 -40.5 不触发、-35/-25/-15 各触发一次。
  可用 VOICE_STARTLE_MIN_DBFS / VOICE_STARTLE_DELTA_DB 覆盖。
未修、需要单独查的（不做盲目下调）：
- gaze_score 用户自认在看镜头时只有 0.42–0.45。把 0.65 降到 0.45 会让「看别处」也通过，
  这是视觉侧的测量问题，不该在注意力层用阈值掩盖。建议在 8092 视觉原始输出行边看边确认实际读数。
- 声纹相似度 0.50–0.59：换 ERes2NetV2 后真人同人相似度明显低于我用合成语音测出的 0.85+。
  我当时的合成语音标定不足以支撑沿用 0.48/0.6，这一点我判断错了。需要真人 ROC。
  在此之前，任何路径都不应单独依赖声纹 ≥0.6。
新增用例：MeasuredThresholdTests（0.82 人脸分必须算在场；无法识别的人不得借用他人历史）。
验证：Attention 182、Brain 72、Vision 23、C++ 4 目标、离线端到端全过。
